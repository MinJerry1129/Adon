...
class SChatDetailVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        c_uid = AppDelegate.shared().clientuser_uid
    ...
    func fetchData() {
        ...
        database.child("chat").child(user_uid).child(chat_uid).child(c_uid).observe(.childAdded) { (snapshot) in
    ...
    @IBAction func onSentMsg(_ sender: Any) {
        if (txtMessage.text == "" || txtMessage.text == " ") {
            ...
            database.child("chat").child(user_uid).child(chat_uid).child(c_uid).child("\(time_interval)").updateChildValues(["message" : txtMessage.text!, "status" : "send" , "date" : time_interval])
            
            database.child("chat").child(c_uid).child(chat_uid).child(user_uid).child("\(time_interval)").updateChildValues(["message" : txtMessage.text!, "status" : "receive" , "date" : time_interval])
            
            database.child("chat").child(c_uid).child(chat_uid).updateChildValues(["status" : "yes"])
            self.chatTB.reloadData()
            txtMessage.text = ""
        }
    }
...
extension SChatDetailVC: UITableViewDataSource {
